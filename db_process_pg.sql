...
BEGIN
  DECLARE log_date DATE DEFAULT (CURRENT_DATE);   # отчётная дата
  IF delta > 0 THEN SET log_date = DATE_SUB(CURRENT_DATE, delta);
  END IF;
...
INSERT INTO log (log_date, time_slot, class, subj, tutor, size) VALUES (DATE_SUB(CURRENT_DATE, 1), 1, 1, 1, (SELECT id FROM tutors WHERE lastName = 'Яндексон' AND firstName = 'Соня' AND patronym = 'Стахановна' ), 11);
...
as $$
DECLARE
log_start_date DATE DEFAULT (CURRENT_DATE - INTERVAL '1 MONTH');
